...
class UserController extends AbstractController
{
    ...
    public function getInfo(Request $request , TokenRepository $tokenRepos , SerializerInterface $serializer): JsonResponse
    {
        ...
        if ($bearerToken) {
            ...
            if(!$token){
                return new JsonResponse(['error' => 'Vous n\'avez pas l\accès. Token Inexistante'], Response::HTTP_UNAUTHORIZED);
            }
            $user = $token->getIdUser();
        } else {
            return new JsonResponse(['error' => 'Vous n\'avez pas l\accès.'], Response::HTTP_UNAUTHORIZED);
        }
    ...
    public function modifyInfo(Request $request , EntityManagerInterface $entity , TokenRepository $tokenRepos , SerializerInterface $serializer , UserPasswordHasherInterface $passwordEncoder , ValidatorInterface $validator): JsonResponse
    {
        // Récupérer le token Bearer depuis l'en-tête Authorization
        $bearerToken = $request->headers->get('Authorization');
        $user = null;
        if ($bearerToken) {
            $bearerToken = str_replace('Bearer ', '', $bearerToken);
            $token = $tokenRepos->findTokenAuth($bearerToken,'AUTH');
            if(!$token){
                return new JsonResponse(['error' => 'Vous n\'avez pas l\accès.'], Response::HTTP_UNAUTHORIZED);
            }
            $user = $token->getIdUser();
        ...
        return new JsonResponse(['message' => 'Information modifié.'],Response::HTTP_ACCEPTED,[],true);
    }
    ...
    public function deleteUser(Request $request , EntityManagerInterface $entity , TokenRepository $tokenRepos , SerializerInterface $serializer , UserPasswordHasherInterface $passwordEncoder , ValidatorInterface $validator): JsonResponse
    {
        // Récupérer le token Bearer depuis l'en-tête Authorization
        $bearerToken = $request->headers->get('Authorization');
        $user = null;
        ...
        return new JsonResponse(['message' => 'Votre compte a été supprimer.'],Response::HTTP_ACCEPTED,[],true);
    }
}
